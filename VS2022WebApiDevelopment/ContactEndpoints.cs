...
public static class ContactEndpoints
{
    public static void MapContactEndpoints (this IEndpointRouteBuilder routes)
    {
        ...
        group.MapGet("/", async (VS2022WebApiDevelopmentContext db) =>
        {
        ...
        group.MapGet("/{id}", async Task<Results<Ok<Contact>, NotFound>> (int id, VS2022WebApiDevelopmentContext db) =>
        {
        ...
        group.MapPut("/{id}", async Task<Results<Ok, NotFound>> (int id, Contact contact, VS2022WebApiDevelopmentContext db) =>
        {
        ...
        group.MapPost("/", async (Contact contact, VS2022WebApiDevelopmentContext db) =>
        {
        ...
        group.MapDelete("/{id}", async Task<Results<Ok, NotFound>> (int id, VS2022WebApiDevelopmentContext db) =>
        {
            var affected = await db.Contact
                .Where(model => model.Id == id)
                .ExecuteDeleteAsync();
